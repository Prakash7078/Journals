...
import csv
import os
data=pd.read_csv('DSS_2025_article_data.csv')
...
with open('DSS_2025_CLEANED.csv', mode='a', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['URL','Journal_Title','Article_Title','Month_Year','Abstract','Keywords','Author_name','Standardized_Author','Author_email','Author_Address','Author_University','Author_department','Author_State','Author_Country','Author_pincode'])
for index,row in data.iterrows():
    address=row['Author_name']+row['Author_Address']
    result = extract_author_address(address)
    with open('DSS_2025_CLEANED.csv', mode='a', newline='',encoding='utf-8') as file:
        writer=csv.writer(file)
        writer.writerow([row['URL'],row['Journal_Title'],row['Article_Title'],row['Month_Year'],row['Abstract'],row['Keywords'],row['Author_name'],result['Standardized_Author'],row['Author_email'],row['Author_Address'],result['University'],result['Department'],result['State'],result['Country'],result['Pincode']])